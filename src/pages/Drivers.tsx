      ...
      <div className="p-6">
          ...
          <TabsContent value="assignments">
            <div className="bg-white rounded-lg shadow-subtle overflow-hidden">
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-100">
                  ...
                  <tbody className="bg-white divide-y divide-gray-100">
                    {driverOrderAssignments && driverOrderAssignments.map((driver, index) => {
            ...
            <div className="mt-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {driverOrderAssignments && driverOrderAssignments.slice(0, 6).map((driver) => {
                  const driverOrders = mockOrders.filter(order => order.driver === driver.name);
                          ...
                          {driverOrders.map(order => (
                            <li key={order.id} className="text-sm">
                              <div className="flex justify-between">
                                ...
                                <Badge variant={order.status === 'in_delivery' ? 'default' : 'outline'}>
                                  {order.status === 'in_delivery' ? 'In Delivery' : 'Picked Up'}
                                </Badge>
                              </div>
                              <div className="text-gray-500 mt-1">{order.customer}</div>
          ...
          <TabsContent value="available">
            <div className="bg-white rounded-lg shadow-subtle overflow-hidden">
              <div className="p-4 border-b border-gray-100">
                ...
                <p className="text-sm text-gray-500 mt-1">
                  Active drivers ready for order assignment
                </p>
              </div>
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-100">
                  ...
                  <tbody className="bg-white divide-y divide-gray-100">
                    {availableDrivers.map((driver) => (
                      <tr key={driver.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <Badge variant={driver.currentTask === 'idle' ? 'outline' : 'default'}>
                            {driver.currentTask === 'idle' ? 'Available' : 'Delivering'}
                          </Badge>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                          {driver.location}
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                          <Button 
                            variant="outline" 
                            size="sm"
                            onClick={() => navigate('/order-assignment')}
                            className="flex items-center gap-1"
                          >
                            <Package className="h-4 w-4" />
                            Assign Order
        ...
        <Dialog open={orderDetailsOpen} onOpenChange={setOrderDetailsOpen}>
          <DialogContent className="sm:max-w-lg">
            <DialogHeader>
              <DialogTitle>Order Details for {selectedDriver?.name}</DialogTitle>
              <DialogDescription>
                ...
                <div className="mb-4">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    ...
                    <div className="flex items-center">
                      ...
                      <span>Total Orders: {selectedDriver.totalOrders}</span>
                    </div>
                  </div>
                </div>
                
                <h4 className="font-medium text-sm mb-2">Assigned Orders</h4>
                <div className="border rounded-md divide-y">
                  {driverOrders.length > 0 ? (
                    driverOrders.map(order => (
                      <div key={order.id} className="p-3">
                        <div className="flex justify-between items-center mb-1">
                          <span className="font-medium">{order.id}</span>
                          <Badge variant={order.status === 'in_delivery' ? 'default' : 'outline'}>
                            {order.status === 'in_delivery' ? 'In Delivery' : order.status}
                          </Badge>
                        </div>
                    ...
                    <div className="p-4 text-center text-gray-500">
                      No orders assigned to this driver from our system
                    </div>
                  )}
                </div>
              </div>
            ...
            {selectedDriver && (
              <div className="mt-4">
                <div className="flex items-center mb-6">
                  ...
                  <div>
                    ...
                    <div className="flex items-center">
                      ...
                      <div className="ml-2 flex items-center text-sm text-yellow-500">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                  ...
                  <div>
                    <h4 className="text-sm font-medium text-gray-500 mb-2">Driver Details</h4>
                    <div className="space-y-2">
                      <div className="flex items-center">
                        <Calendar className="h-4 w-4 mr-2 text-gray-500" />
                        <span>Joined: {selectedDriver.joinDate}</span>
                      ...
                      {selectedDriver.vehicleInfo && (
                        <div className="flex items-center">
                          ...
                          <span>{selectedDriver.vehicleInfo}</span>
                        </div>
                      )}
                ...
                <div>
                  ...
                  <div className="bg-gray-50 p-3 rounded-md">
                    <div className="flex items-center mb-2">
                      <Badge variant={selectedDriver.currentTask === 'idle' ? 'outline' : 'default'}>
                        {selectedDriver.currentTask === 'idle' ? 'Available' : 'On Delivery'}
                      </Badge>
                      <span className="ml-2 text-sm text-gray-500">
                        {selectedDriver.currentTask === 'idle' 
                          ? 'Driver is available for new orders' 
                          : `Currently delivering order ${selectedDriver.currentOrder}`
                        }
                      </span>
                    </div>
                    <div className="text-sm">
